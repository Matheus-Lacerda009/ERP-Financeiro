...
public class Main {
    public static void main(String[] args) throws NomeInvalidoException {
        ...
        Connection conn = Conexao.connecting(USER, PASSWORD);
        if(conn != null) {
            Categoria_ItemService categoria_itemService = new Categoria_ItemService(conn);
            ProdutosService produtosService = new ProdutosService(conn);
            int opc;
            do {
