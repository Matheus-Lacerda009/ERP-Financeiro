...
import net.financeiro.service.Valor_pVendaService;

import java.util.ArrayList;
import java.util.List;

public class Valor_pVendaMenu implements TableMenu {
    //Paleta e styles
    ...
    private final Valor_pVendaService service = new Valor_pVendaService();
    ...
    List<Valor_pVenda> itens = new ArrayList<>();

    public Valor_pVendaMenu() {
        this.viewport = new Viewport();
    }

    @Override
    public String getTitle() {
        return "Valor por venda";
    }
    ...
    private Command consultarBanco() {
        return () -> {
            try {
                List<Valor_pVenda> result = service.visualizar();
    ...
    private void atualizarViewport() {
        ...
        for(int i = 0; i < itens.size(); i++) {
            Valor_pVenda item = itens.get(i);
            String prefix = (i == cursor) ? " > " : "   ";

            String linha = String.format("%s%-5d %-10f\n", prefix, item.getId_operacao(), item.getValor());
