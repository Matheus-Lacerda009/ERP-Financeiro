...
public class FuncionarioMenu implements TableMenu {
            ...
            //Ações caso o estado seja EDITANDO ou CRIANDO
                ...
                switch(k) {
                    ...
                    case "ctrl+s":
                        ...
                        final String cpf = inputs.get(1).value();
                        final String telefone = inputs.get(2).value();
                        final String email = inputs.get(3).value();
                        ...
                        return () -> {
                            try {
                                Funcionario item = new Funcionario(nome, cpf, telefone, email);
                                if(modoOperacao == Estado.CRIANDO) {
                                    service.inserir(item);
                                } else if(modoOperacao == Estado.EDITANDO) {
                                    final long idItem = itens.get(idSelecionado).getId_funcionario();
                ...
                switch(k) {
                    case "ctrl+s":
                        ...
                        return () -> {
                            try {
                                //deleta a entidade
                                service.deletar(itens.get(idSelecionado).getId_funcionario());
    ...
    private void iniciarFormulario() {
        ...
        if (estado == Estado.EDITANDO) {
            ...
            inputs.get(1).setValue(item.getCpf());
            inputs.get(2).setValue(item.getTelefone());
            inputs.get(3).setValue(item.getEmail());
