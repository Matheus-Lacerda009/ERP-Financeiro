...
public class Forma_PagamentoMenu implements TableMenu {
    //Paleta e styles
    ...
    List<Forma_Pagamento> itens = new ArrayList<>();

    public Forma_PagamentoMenu() {
    ...
    private Command consultarBanco() {
        return () -> {
            try {
                List<Forma_Pagamento> result = service.listarInfo();
            ...
            //Ações caso o estado seja EDITANDO ou CRIANDO
                ...
                switch(k) {
                    ...
                    case "ctrl+s":
                        ...
                        return () -> {
                            try {
                                Forma_Pagamento item = new Forma_Pagamento(nome);
                                if(modoOperacao == Estado.CRIANDO) {
                                    service.inserir(item);
                                } else if(modoOperacao == Estado.EDITANDO) {
                                    final long idItem = itens.get(idSelecionado).getId_forma_pagamento();
                ...
                switch(k) {
                    case "ctrl+s":
                        ...
                        return () -> {
                            try {
                                //deleta a entidade
                                service.deletar(itens.get(idSelecionado).getId_forma_pagamento());
    ...
    public void onDataReceived(Object data, String error) {
        if(data == null) {
            ...
            if(data instanceof List<?>) {
                itens = (List<Forma_Pagamento>) data;
                cursor = 0;
                viewport.gotoTop();
                atualizarViewport();
            }
        ...
        switch(estado) {
            ...
            case Estado.VISUALIZANDO:
                for(int i = 0; i < itens.size(); i++) {
                    Forma_Pagamento item = itens.get(i);
                    String prefix = (i == cursor) ? " > " : "   ";

                    String linha = String.format("%s%-5d %-20s\n", prefix, item.getId_forma_pagamento(), item.getNome());
    ...
    private void iniciarCampos() {
        ...
        nomeInput.setPlaceholder("Ex: PIX");
    ...
    private void iniciarFormulario() {
        ...
        if(estado == Estado.EDITANDO) {
            ...
            Forma_Pagamento item = itens.get(cursor);
